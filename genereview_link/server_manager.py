    ...
    async def create_mcp_server(self, app: FastAPI, config: ServerConfig) -> FastMCP:
        ...
        mcp = FastMCP.from_fastapi(
            ...
            instructions=(
                ...
                "section. Citation contract: every claim must cite "
                ...
                "brief-mode payloads tight. Empty search_passages results "
                "carry rule-based hints in _meta.diagnostics.suggestions "
                "(e.g. gene-filter-kills-hits, broaden-long-query, "
                "section-filter-drops-all); inspect that field before "
                "retrying with looser parameters. get_passage now accepts "
                "neighbors=N (0..5) and cross_sections=true|false; the "
                "response is always wrapped as {passage, neighbors_before, "
