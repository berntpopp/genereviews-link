...
async def get_fulltext(
    ...
    try:
        ...
        if result.get("error"):
            raise HTTPException(status_code=404, detail=f"Could not scrape content: {result['error']}")
        ...
        raise HTTPException(status_code=500, detail="An error occurred while scraping the full text.")