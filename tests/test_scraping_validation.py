...
class TestDataIntegrity:
    ...
    def test_brca1_content_accuracy(self, client):
        ...
        assert (
            len(found_terms) >= 6
        ), f"Should find BRCA-specific terms, found: {found_terms}"

        # Should not contain irrelevant content
        irrelevant_terms = ["javascript", "advertisement", "cookie policy"]
        for term in irrelevant_terms:
            assert (
                term not in all_content
            ), f"Should not contain irrelevant content: {term}"
        ...
        for fixture in fixtures:
            ...
            for section_key, section_data in sections.items():
                ...
                for field in required_fields:
                    ...
                    ), f"Section {section_key} missing field: {field}"

                # Field types
                assert isinstance(
                    section_data["title"], str
                ), f"Section {section_key} title should be string"
                assert isinstance(
                    section_data["content"], str
                ), f"Section {section_key} content should be string"
                assert isinstance(
                    section_data["level"], int
                ...
                for sub_key, sub_data in section_data["subsections"].items():
                    ...
                    assert (
                        "content" in sub_data
                    ), f"Subsection {sub_key} missing content"
    ...
    def test_content_formatting_consistency(self, client):
        ...
        for fixture in fixtures:
            ...
            for section_key, section_data in sections.items():
                ...
                assert "<" not in content or not re.search(
                    r"<[^>]+>", content
                ), f"Section {section_key} contains HTML tags"

                # Should not have control characters
                assert not re.search(
                    r"[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F]", content
...
class TestContentCompleteness:
    ...
    def test_section_coverage(self, client):
        ...
        coverage_ratio = (
            extracted_count / original_h2_count if original_h2_count > 0 else 0
        )
        assert (
            coverage_ratio >= 0.7
        ...
        if has_subsections:
            ...
            for section_key, section_data in sections.items():
                ...
                for sub_key, sub_data in subsections.items():
                    ...
                    assert (
                        len(sub_data["content"]) > 0
                    ), f"Subsection {sub_key} should have content"
...
class TestRegressionPrevention:
    ...
    def test_navigation_menu_exclusion(self, client):
        ...
        for keyword in navigation_keywords:
            assert (
                keyword not in all_content
            ), f"Navigation content should be excluded: '{keyword}'"
    ...
    def test_footer_content_exclusion(self, client):
        ...
        footer_found = [
            keyword for keyword in footer_keywords if keyword in all_content
        ]

        # Allow some copyright notices in medical content, but not extensive footer text
        assert (
            len(footer_found) <= 1
        ), f"Footer content should be minimal: found {footer_found}"
    ...
    def test_duplicate_content_prevention(self, client):
        ...
        for i, (key1, content1) in enumerate(section_contents):
            for j, (key2, content2) in enumerate(section_contents[i + 1 :], i + 1):
